...
int main(int argc, char* argv[])
{
    ...
    {   
        ...
        {   
            ...
            clean_stdin();
            functReturnVal = addRecord(&start, accNum, accName, accAddr);
            if (functReturnVal == 0)
            {
                printf("\n-- add of account# '%d' sucessful --\n",accNum);
            }
            else if (functReturnVal == -1)
            {
                printf("\n-- account  of '%d' already exists --\n",accNum);
            } 
        }
        ...
        else if (strncmp(usrInp, "delete",usrInpSize) == 0)
        {
            ...
            {
                printf("\n-- delete of Account# '%d' does not exist --\n",accNum);
            } 
