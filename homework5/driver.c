...
int main(int argc, char* argv[])
{
    ...
    printf("reading file output.txt to empty array SAME SIZE...\n");
    if (readfile(pokemonbankT,&numpokemons,"output.txt") == 0)
    {
        printf("\nfile sucessfully read!\n");
        printf("pokemon present: %i\ndata in array:\n",numpokemons);
        itrSize = sizeof(pokemonbankT)/sizeof(pokemonbankT[0]);

        for (itr=0; itr < itrSize; itr++)
        {
            printf("%i\n%s\n",pokemonbankT[itr].level,pokemonbankT[itr].name);
        }
    }
