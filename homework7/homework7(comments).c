...
void makeheader(const unsigned char tcpheader[], unsigned char responseheader[])
{
    ...
    /*source bit swap, swap source and destination*/
    if (source3 > (0x7FFF))
    {
        unsigned int result;    
        unsigned int xor;
    ...
    /* WORKING acknowledgement to sequence of request*/
    responseheader[8] = tcpheader[4];
    responseheader[9] = tcpheader[5];
    responseheader[10] = tcpheader[6];
    responseheader[11] = tcpheader[7];    
    
    /* sequence number to the sequence number of the request header AND increment*/
    seq1 = tcpheader[4];
    seq2 = tcpheader[5];
    seq3 = tcpheader[6];
    seq4 = tcpheader[7];
    ...
    printf("\n header %x",responseheader[4]);
    printf("\n header %x",responseheader[5]);
    printf("\n header %x",responseheader[6]);
    printf("\n header %x",responseheader[7]);
    

    /* WORKING If SYN is set to 1 in the request header, both SYN and ACK in the response header must be set to 1*/
    if (cntrl & (1 << 1))
    {
        maskSyn = 0x12;
        responseheader[13] = (cntrl | maskSyn);
        
        printf("CHANGE SYN ACK is %x",responseheader[13]);
    }
